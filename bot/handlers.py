...
from config import BREWERY_CONTROL_URL
...
def view_tanks(update, context):
    ...
    response = requests.get(f'{BREWERY_CONTROL_URL}tank/tanks-view')
    ...
    for diagram in diagrams['diagrams']:
        ...
        context.bot.send_photo(
            chat_id=chat_id,
            photo=open('diagram.png', 'rb'),
            reply_markup=get_full_tank_info(diagram)
            )
...
def view_full_tank_info(update, context):
    ...
    response = requests.get(
        f'{BREWERY_CONTROL_URL}tank/tanks-info',
        params=params
        )
    tank_info = response.json()
    update.callback_query.edit_message_caption(
        caption=format_tank_info(tank_info),
